...
class Service(object):
    ...
    def find_kudo(self, repo_id):
        kudo = self.repo_client.find({'user_id': self.user_id, 'id': repo_id})
    ...
    def update_kudo_with(self, repo_id, githubRepo):
        records_affected = self.repo_client.update({'user_id': self.user_id, 'id': repo_id}, githubRepo)
        return records_affected > 0

    def delete_kudo_for(self, repo_id):
        records_affected = self.repo_client.delete({'user_id': self.user_id, 'id': repo_id})
        return records_affected > 0
